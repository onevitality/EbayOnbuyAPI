    ...
    for o in orders:
        ...
        if isinstance(prods, str):
            try:
                ...
                prods = []
        for p in prods:
            rows.append({"sku": p.get("sku"), "onbuy_quantity": int(p.get("quantity",0))})
    df = pd.DataFrame(rows)
    return df.groupby("sku", as_index=False)["onbuy_quantity"].sum() if not df.empty else df
...
def process_ebay(orders):
    ...
    for o in orders:
        for li in o.get("lineItems",[]):
            rows.append({"sku": li.get("sku"), "ebay_quantity": int(li.get("quantity",0))})
    df = pd.DataFrame(rows)
    return df.groupby("sku", as_index=False)["ebay_quantity"].sum() if not df.empty else df
...
def build_report_df():
    ...
    mapping = mapping.rename(columns={"sku":"sku","product_name":"product_name"}).drop_duplicates(["sku"])
    df = pd.merge(df, mapping, on="sku", how="left")
    df["product_name"] = df["product_name"].fillna("unknown")
    # 4) filter sold only
    df = df.loc[df["total_quantity"]>0, ["product_name","onbuy_quantity","ebay_quantity","total_quantity","sku"]]
